...
- **Schéma formalisé (PLAN.md Étape 9, étendu PLAN-BURST.md Étape 1, Étape 18)** : `{ rules: RotationRule[] }`, où chaque `RotationRule` est `{ spellId: number, conditions: RotationCondition[] }` — une règle sans conditions (`conditions: []`) matche toujours (façon d'exprimer un "sinon → sort par défaut", généralement en dernière position). Les conditions d'une même règle sont en ET logique ; un "OU" s'exprime en dupliquant la règle (même `spellId`, conditions différentes). Cinq types de conditions supportés :
